...
my $rfluxmtxName = (split(/\s+/, $rfluxmtx))[0]; # The name of the program to call
my $accelPrefix = "accelerad_";
my $hasWhich = eval {
	# Check that which command is available
	require File::Which;
	File::Which->import();
	1;
};
if($hasWhich) {
	if (which($accelPrefix . $rfluxmtxName)) { # Returns true if the Accelerad version exists
		$rfluxmtx = $accelPrefix . $rfluxmtx; # Prepends the system call with the Accelerad prefix
	}
} else {
	print STDERR $@; # Print the error generated by running the code in $hasWhich
	print STDERR "Calling $rfluxmtxName instead of $accelPrefix$rfluxmtxName\n";
}
# Get options
while ($#ARGV >= 0) {
	if ("$ARGV[0]" =~ /^[-+]m/) {
