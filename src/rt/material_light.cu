...
RT_CALLABLE_PROGRAM PerRayData_radiance closest_hit_light_radiance(IntersectData const&data, PerRayData_radiance prd)
{
	...
	if (!directvis || 0.0f > data.mat.params.l.maxrad && prd.depth > 0 || prd.ambient_depth > 0 || spotout(data) || dot(data.world_shading_normal, data.ray_direction) > 0.0f) //TODO need a better ambient test
...
RT_METHOD int spotout(const IntersectData &data)
{
	if (data.mat.params.l.siz < -FTINY)
